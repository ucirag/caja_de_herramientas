...
**Gráfico 1. Cantidad de Ingresos totales a internación por otras causas y por IRAG/IRAG Extendida por Semana Epidemiológica. UCIRAG `r nombre_establecimiento_centinela`,`r nombre_provincia`SE `r SE_MIN_UCI` a SE`r SE_MAX_UCI`. Año `r anio_de_analisis`. N= `r n_tabla_agrupado1`**
...
**Gráfico 1. Distribucion porcentual semanal de Ingresos totales a internación por otras causas y por IRAG/IRAG Extendida por Semana Epidemiológica. UCIRAG `r nombre_establecimiento_centinela`,`r nombre_provincia`SE `r SE_MIN_UCI` a SE`r SE_MAX_UCI`. Año `r anio_de_analisis`. N= `r n_tabla_agrupado1`**
...
**Figura 2: Casos acumulados y % de positividad de IRAG notificados, estudiados por laboratorio y confirmados para SARS-CoV-2, Influenza y/o VSR, según agente etiológico. UCIRAG** `r nombre_establecimiento_centinela`**,**`r nombre_provincia`**SE `r SE_MIN_UCI` a SE`r SE_MAX_UCI`. Año `r anio_de_analisis`. N= `r N_testeos_UCI`.**
...
**Figura 3: Distribución temporal de Casos de IRAG estudiados por laboratorio para SARS-CoV-2, Influenza y/o VSR, por semana epidemiológica. UCIRAG** `r nombre_establecimiento_centinela`**,** `r nombre_provincia`**SE `r SE_MIN_UCI`/`r anio_de_analisis` a SE `r SE_MAX_UCI`/`r anio_de_analisis`.**
...
**Figura 7: Distribución por grupo de edad de Casos de IRAG estudiados por laboratorio para SARS-CoV-2, Influenza y/o VSR,. UCIRAG `r nombre_establecimiento_centinela`,** `r nombre_provincia`**SE `r SE_MIN_UCI`/`r anio_de_analisis` a SE `r SE_MAX_UCI`/`r anio_de_analisis`.**

```{r}
#| echo: false
  
casos_gru_edad_graf <- casos_gru_edad_graf %>% select(-AÑO)
highchart() %>%
...
**Figura 10: Combinaciones de signos y síntomas registrados en casos positivos de IRAG confirmados por laboratorio, UCIRAG `r nombre_establecimiento_centinela`, SE `r SE_MIN_UCI`/`r anio_de_analisis` a SE `r SE_MAX_UCI`/`r anio_de_analisis`. N= `r N_testeos_positivos_UCI`**
...
**Figura 11: Combinaciones de comorbilidades registradas en casos positivos de IRAG confirmados por laboratorio, UCIRAG `r nombre_establecimiento_centinela`, SE `r SE_MIN_UCI`/`r anio_de_analisis` a SE `r SE_MAX_UCI`/`r anio_de_analisis`. N= `r N_testeos_positivos_UCI`**
