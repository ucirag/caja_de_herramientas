...
Dentro de este marco, se realizó el presente reporte, el cual contiene un análisis de la información epidemiológica, clínica y de laboratorio de la estrategia de vigilancia centinela de infecciones respiratorias agudas graves (IRAG) del Hospital `r nombre_establecimiento_centinela`, de la provincia de `r PROVINCIA`, desde la SE `r SE_MIN_UCI`al `r SE_MAX_UCI` del año `r max_anio`.\
...
**Figura 1: Mapa de establecimiento de salud d ela provincia: Distribución geográfica y caracterización según tipo . `r nombre_provincia`, `r mes_SE_MAX_UCI_nombre` año `r mes_SE_MAX_UCI_nombre`.**
...
Gráfico 1. Ingresos totales a internacion por otras causas y por IRAG por semana y Año. Hasta `r ultima_semana_valida`.N= Ver que n poner
